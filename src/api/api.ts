...
export const createAxiosInstance = () => {
    ...
    instance.interceptors.request.use(
        (config) => {
            const token = store.getState().tokenSlice.token;
            if (token) {
...
export const authApi = {
    getToken: (initData: string) => {
        return axiosInstance.get('/users/login', { params: { initData: initData } });
    },
    getMyData: () => {
    ...
    createWithdrawReq: (data: { sum: string, operation: string, comment: string }) => {
        return axiosInstance.post('/users/balance-req', data)
    },
    createFeedback: (data: { telegram: string, text: string }) => {
...
server {
        ...
        root /var/www/html;
         ...
         # Add index.php to the list if you are using PHP
        index index.html index.htm index.nginx-debian.html;
    server_name web-app-telegramm-backend.ru; # managed by Certbot
    ...
    ssl_certificate /etc/letsencrypt/live/web-app-telegramm-backend.ru/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/web-app-telegramm-backend.ru/privkey.pem; # managed by Certbot
    include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot

}
server {
    if ($host = web-app-telegramm-backend.ru) {
        ...
        listen 80 ;
        listen [::]:80 ;
    server_name web-app-telegramm-backend.ru;
